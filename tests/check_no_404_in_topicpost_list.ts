...
let urls: string[];
test.serial("can get topic post first page list urls", async (t) => {
    const res = await httpClient.get("/fuwu");
...
test.serial("http get urls from topic post list concurrently", async (t) => {
    t.timeout(5 * 60 * 1000);
    const errors: string[] = [];
